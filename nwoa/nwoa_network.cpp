...
::nwol::error_t								runCommunications						(::gddm::SFrameworkNetworkClient& appNetwork)											{
	...
	while gbit_true(appNetwork.State, ::gddm::NETWORK_STATE_ENABLED) {
		be_if(result = ::gdnet::ping(instanceClient.Endpoints) ? 0 : -1, "%s", "Ping timeout.");	// Ping before anything else to make sure everything is more or less in order.
		::nwol::sleep(500);

		uint64_t										current_time;
		be_if(errored(result = ::gdnet::time(instanceClient.Endpoints, current_time)), "Failed to get server time.");		// get server time
