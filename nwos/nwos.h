...
struct SApplication	: public ::gddm::SFramework {
	NWOM(::, SApplication, ::gdnet, SServer,	NetworkServer, ::nwol::GDATA_TYPE_OBJ, "Network server", "Holds server connection state and client information.")	= {};	// 
	NWOM_REGISTRY
		(	NWOM_NAME(NetworkServer)
		);
	inline										SApplication			(::nwol::SRuntimeValues	* runtimeValues)						: SFramework(runtimeValues)	{ GUI = {{640, 480}, {132 >> 1, 60 >> 1}}; }
};
