...
//-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	nwol_necall(::networkDisable(instanceApp), "Failed to disable network.");
	...
	error_if(errored(::nwol::asciiDisplayTitleSet({moduleTitle, moduleTitleLen})), "Failed to set console title.");
	...
	//nwol_necall(::setupREST	(instanceApp), "%s", "Failed to set up REST."		);
...
template<uint32_t _screenWidth, uint32_t _screenHeight>
	...
	for(uint32_t y = 0, yMax = ::nwol::min(_screenHeight, target.Height()); y < yMax; ++y)
		for(uint32_t x = 0, xMax = ::nwol::min(_screenWidth, target.Width()); x < xMax; ++x)
			target.Characters[y][x]							= source.Cells[y][x];
}
	...
	::nwol::SASCIITarget							& renderTarget					= instanceApp.ASCIITarget;
	::nwol::asciiTargetClear(renderTarget);
	::bltASCIIScreen(targetScreenTTT, renderTarget);
	::nwol::renderGUIASCII(renderTarget, instanceApp.GUI);
	::nwol::asciiDisplayPresent(renderTarget);
	verbose_printf("%s", "Exit.");
	return 0; 
}
