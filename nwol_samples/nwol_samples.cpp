...
int32_t											cleanup											(::SApplication& instanceApp)																									{ 
	...
	nwol_necall(::nwol::asciiDisplayDestroy(), "Why would this ever happen? Most likely the display wasn't initialized...");
	nwol_necall(::nwol::asciiTargetDestroy(instanceApp.ASCIITarget), "Why would this ever happen? Most likely the target wasn't initialized...");
	PLATFORM_CRT_CHECK_MEMORY();
	return 0; 
}
...
int32_t											loadValidModules								(const char* modulesPath, ::nwol::SRuntimeValues* runtimeValues, ::nwol::array_pod<::nwol::SApplicationModule>& loadedModules)	{
	...
	for(uint32_t iFile = 0, fileCount = possibleModuleNames.size(); iFile < fileCount; ++iFile) {
		...
		cw_if(errored(errorLoad), "Module is not a valid NWOR interface: %s.", moduleName.begin());
...
::nwol::error_t									listDLLFiles									(const char* modulesPath, ::nwol::array_obj<::nwol::glabel>& possibleModuleNames)												{
	::nwol::array_obj<::nwol::glabel>					fileNames;
...
#	error "Not implemented."
		...
		if(0 > errSetup) { 
			...
			retVal											= -1;
			error_if(errored(moduleInterface.Cleanup()), "Cleanup() failed!");
	...
	case SELECTOR_STATE_RUNNING_SELECTION	:
		...
		error_if(errored(updateResult = moduleSelected->Update(exitRequested)), "Module \"%s\" failed to update with error code %u.", moduleTitle, updateResult)
			...
			if(callbackPointersErased) { 
				...
				nwol_necall(::nwol_moduleVersion(&selectorModuleVersion), "??");
				nwol_necall(::nwol_moduleTitle(selectorModuleTitle, &selectorModuleTitleLen), "If this fails then something weird is going on.");
				::sprintf_s(windowTitle, "%s v%u.%u", selectorModuleTitle, (selectorModuleVersion & 0xFF00U) >> 8, selectorModuleVersion & 0xFFU);
				nwol_necall(::nwol::asciiDisplayTitleSet(windowTitle), "Unknown error.");
#if defined(__WINDOWS__)
				SetWindowText(instanceApp.RuntimeValues->Screen.PlatformDetail.hWnd, windowTitle);
#else
...
int32_t											renderSelectorApp								(const ::SApplication& instanceApp)																								{
	...
	nwol_necall(::nwol::asciiTargetClear(target), "If this happens then it's likely that memory was corrupted somehow or maybe the target wasn't properly initialized/was already shut down.");
...
int32_t											render											(::SApplication& instanceApp)																									{
	int32_t												retVal											= 0;
	if(1 == sync_increment(instanceApp.RenderSemaphore)) {	// sync here. This is necessary for when the client application requests shutdown but may still be rendering a frame, so we have to wait the render call to finish before deleting the object.
		switch(instanceApp.SelectorState) {
